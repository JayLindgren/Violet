...
#  else
   if ( ( uint32_t )b )
   {
      _BitScanForward( &x, ( unsigned long )b );
   }
   else
   {
      _BitScanForward( &x, ( unsigned long )( b >> 32 ) );
      x += 32;
   }
#           endif
   return ( int ) x;
...
#  else
   if ( b >> 32 )
   {
      _BitScanReverse( &x, ( unsigned long )( b >> 32 ) );
      x += 32;
   }
   else
   {
      _BitScanReverse( &x, ( unsigned long )b );
   }
#           endif
   return ( int ) x;
}
...
void decode_fen( const char * fen_str,
                 int * player,
                 int * castle,
                 int * fifty,
                 int * move_number,
