...
// Your ComputerMove() function should call this to check if the search is
// restricted.
...
void SendInfoCommand( int depth, int seldepth, int score, long long nodes,
                      unsigned long time_ms, const std::string &pv )
{
   ...
   if ( score > 90000 )
   {
      int movesToMate = ( 99999 - score + 1 ) / 2;
      scoreStr        = "mate " + std::to_string( movesToMate );
   }
   else if ( score < -90000 )
   {
      int movesToMate = ( -99999 - score ) / 2;
      scoreStr        = "mate " + std::to_string( movesToMate );
   }
   else
   {
   ...
   if ( pv.empty() )
   {
      cout << "info depth " << depth << " seldepth " << seldepth << " score " << scoreStr << " nodes " << nodes
           << " nps " << nps << " time " << time_ms << endl;
   }
   else
   {
      cout << "info depth " << depth << " seldepth " << seldepth << " score " << scoreStr << " nodes " << nodes
           << " nps " << nps << " time " << time_ms << " pv " << pv << endl;
   }
}
...
void SendBestMoveCommand( const std::string &bestMove,
                          const std::string &ponderMove )
{
   ...
   cout << endl;
   cout.flush(); // FIX: Ensure output is immediately visible to UCI interface
}
...
void InitializeCommunications()
{
   ...
   if ( GetInterfaceDebug() )
   {
      ...
      gofDebug << endl;
      gofDebug << "Interface started from interface." << endl
               << endl;
   }
}
...
int CheckForInput()
{
   ...
   if ( giPipe == 0 )
   {
      if ( GetNumberOfConsoleInputEvents( ghStandardInput, &dwInput ) )
      {
   ...
   {
      // For pipes, PeekNamedPipe is used. It checks for data in the pipe without
      // blocking.
                          ...
                          struct GeneralMove *argsGeneralMoves )
{
   ...
   // Read a complete line from stdin. This is more reliable than a fixed-size
   // buffer.
   if ( getline( cin, line ) )
   {
      if ( gofDebug.is_open() )
      ...
      // std::cout << "ReadInputAndExecute: " << line << std::endl;
...
{
   // Send a command to the interface. Notice the end of line character is now
   // handled by endl. Always use this routine to send a command to make sure we
   // have the end of line.

   // Debug.
   if ( GetInterfaceDebug() )
   {
      gofDebug << "Sending a command to the interface = " << argstrCommand << endl
               << endl;
   }
...
//
//
void CommandFromInterface( std::string argstrCommand, struct Board *argsBoard,
                           struct GeneralMove *argsGeneralMoves )
{
   // Debug the inputs.
   assert( argsBoard >= 0 );
   ...
   if ( GetInterfaceDebug() )
   {
      gofDebug << "Processing command: " << command
               << " | Full line: " << argstrCommand << endl
               << endl;
   }
   ...
   if ( command == "uci" )
   {
      ...
      SendCommand( "option name Move Ordering type spin default 0 min 0 max 1" );
   ...
   else if ( command == "setoption" )
   {
      ...
      if ( name == "Clear Hash" )
      {
         ClearHashTable();
         // Optional: notify GUI
         SendCommand( "info string Transposition table cleared" );
      }
      else if ( name == "Hash" )
      {
         if ( !value.empty() )
         {
            ...
            if ( hash_size_mb < 1 )
               hash_size_mb = 1;
            if ( hash_size_mb > 2048 )
               hash_size_mb = 2048;

            // Resize TT and clear it
            SetHashTableSizeMB( hash_size_mb );
            ClearHashTable();

            // Optional: acknowledge
            std::stringstream ack;
            ack << "info string Hash set to " << hash_size_mb << " MB";
            SendCommand( ack.str().c_str() );
         }
      }
      else if ( name == "Move Ordering" )
      {
         int mode = 0;
         try
         {
         ...
         if ( mode > 1 )
            mode = 1;
         gsTempus.giMoveOrderingMode = mode;

         std::stringstream ack;
         ack << "info string Move Ordering set to "
             << ( mode == 0 ? "A (Baseline)" : "B (Experimental)" );
         SendCommand( ack.str().c_str() );
      }
      else if ( name == "Ponder" )
      {
      ...
      else if ( name == "Threads" )
      {
         ...
         std::cout << "CommandFromInterface: Calling InitializeThreads(" << threads
                   << ")" << std::endl;
         InitializeThreads( threads );
         std::cout << "CommandFromInterface: InitializeThreads returned"
                   << std::endl;

         std::stringstream ack;
         ack << "info string Threads set to " << threads;
         SendCommand( ack.str().c_str() );
      }
      else if ( name == "SyzygyPath" )
      {
         bool              success = tb_init( value.c_str() );
         std::stringstream ack;
         if ( success )
            ack << "info string SyzygyPath set to " << value << ". TB_LARGEST: " << TB_LARGEST;
         else
            ack << "info string SyzygyPath failed to init with path: " << value;
         SendCommand( ack.str().c_str() );
      }
      else if ( name == "UCI_ShowCurrLine" || name == "UCI_ShowRefutations" ||
                name == "UCI_LimitStrength" || name == "UCI_Elo" ||
                name == "UCI_AnalyseMode" || name == "UCI_Opponent" ||
                name == "UCI_EngineAbout" || name == "UCI_ShredderbasesPath" ||
                name == "UCI_SetPositionValue" )
      {
         // Ignore these options for now as they are not yet supported or relevant
      }
   }
   else if ( command == "register" )
   {
      // We don't need registration, so just say later or ok
      std::string arg;
      ss >> arg;
      if ( arg == "later" )
      {
      ...
      // Always respond with checked if requested, but spec says register command
      // is sent BY GUI. The engine sends "registration checking" then
      // "registration ok" if it needs it. Since we don't need it, we can ignore
      // or just confirm.
   }
   else if ( command == "ucinewgame" )
   {
      CreateBoard( argsBoard, argsGeneralMoves );
      // Per UCI, just clear TT for a new game; keep current size and keys
      ClearHashTable();
   }
   ...
   else if ( command == "testpv" )
   {
      TestPVBug( argsBoard, argsGeneralMoves );
   }
   else if ( command == "testhasht" )
   {
      TestHashTablePerformance( argsBoard, argsGeneralMoves );
   }
   else if ( command == "testnull" )
   {
      TestNullMovePerformance( argsBoard, argsGeneralMoves );
   }
   else if ( command == "testmoveordering" )
   {
      std::string arg;
      bool        forceBaseline = false;
      if ( ss >> arg )
      {
         if ( arg == "full" || arg == "reset" )
         {
            forceBaseline = true;
         }
      }
      TestMoveOrdering( argsBoard, argsGeneralMoves, false, forceBaseline );
   }
   else if ( command == "tunemoveordering" )
   {
      TuneMoveOrderingB( argsBoard, argsGeneralMoves );
   }
   else if ( command == "tunemoveparameters" )
   {
      TuneMoveParameters( argsBoard, argsGeneralMoves );
   }
   else if ( command == "testsee" )
   {
...
//
void PositionCommand( std::string argstrCommand, struct Board *argsBoard,
                      struct GeneralMove *argsGeneralMoves )
{
   std::stringstream ss( argstrCommand );
   std::string       token;
   ...
   if ( token == "moves" )
   {
      ...
      {
         // Skip invalid moves (e.g., "0000", garbage tokens)
         // Check length AND that it starts with a letter 'a'-'h'
         if ( token.length() < 4 || token.length() > 5 ||
              token[ 0 ] < 'a' || token[ 0 ] > 'h' )
         {
            if ( GetInterfaceDebug() )
            {
               gofDebug << "Skipping invalid move token: " << token << endl;
            }
            continue;
         }
         ...
         FindAlgebraicMove( argsBoard, argsGeneralMoves, moveStr, vsMoveList,
                            &iMoveNumber );
...
//
void GoCommand( std::string argstrCommand, struct Board *argsBoard,
                struct GeneralMove *argsGeneralMoves )
{
   std::cout << "GoCommand: " << argstrCommand << std::endl;
   // Debug the inputs.
   assert( argsBoard >= 0 );
   assert( argsGeneralMoves >= 0 );
   ...
   {
      if ( token == "wtime" )
         ss >> token, SetWhiteTime( std::stoi( token ) );
      else if ( token == "btime" )
         ss >> token, SetBlackTime( std::stoi( token ) );
      else if ( token == "winc" )
         ss >> token, SetWhiteIncrementalTime( std::stoi( token ) );
      else if ( token == "binc" )
         ss >> token, SetBlackIncrementalTime( std::stoi( token ) );
      else if ( token == "movestogo" )
         ss >> token, SetMovesToGo( std::stoi( token ) );
      else if ( token == "depth" )
      {
         ss >> token;
      ...
      else if ( token == "nodes" )
      {
         ...
         ss >> token, SetMate( std::stoi( token ) );
   ...
   // --- Time Management ---
   if ( GetSearchDepth() == dInfiniteDepth &&
        GetSearchTimeInMiliSeconds() == dInfiniteTime && GetNodes() == 0 )
   {
