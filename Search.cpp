                 ...
                 int argiBeta )
{
   ...
   for ( int iDepth = 0; iDepth < GetSearchDepth(); iDepth++ )
   {
      ...
      if ( GetStopGo() == dStop )
      {
         ...
         delete sOldBoard; // FIX: Use delete instead of free for consistency
         return iScore;
                 ...
                 int* argiBestMoveSearched )
{
   ...
   int* viMoveScores = new int[dNumberOfMoves];
   int iMoveCount;
   char strUpdate[1280];
   char strMove[64];
   int iHaveMove = 0;
            ...
            if (argiAlpha > oldAlpha)
            {
               ...
               *argsBoard = tempBoard;
               ...
               if (GetInterfaceMode() == dUCI)
               {
                  ...
                  // Create and send the PV string update...
                  strcpy(strPV, "");
                  for (int iPVCount = 0; iPVCount <= argsBoard->iMaxPlys; iPVCount++)
                  {
                     CreateAlgebraicMove(strMove, &argsBoard->vmPrincipalVariation[0][iPVCount], 0);
                  ...
                  sprintf(strUpdate,
                     "info depth %d seldepth %d nodes %lld pv%s score cp %d",
...
# endif
   ...
   if (GetUseNullMove())
   {
                        ...
                        vsMoveList ))
      {
         ...
         // Create a local copy of the board using consistent memory management
         Board* sNullBoard = new Board();
         *sNullBoard = *argsBoard;
         ...
         // Extract the old hash state
         oldHash = GetUseHashTable();

         // IMPORTANT: Set the last move as null to prevent double null moves
         int iOldLastMoveNull = sNullBoard->iLastMoveNull;
         sNullBoard->iLastMoveNull = dYes;

         // Reduce the depth of the search.
         int iNullReduction = GetNullReduction();
         sNullBoard->iMaxPlys = argsBoard->iMaxPlys - iNullReduction;

         // Ensure we still have a valid depth after reduction
         if ( sNullBoard->iMaxPlys <= sNullBoard->iNumberOfPlys )
         {
            delete sNullBoard;
            return argiAlpha; // Depth reduction too aggressive
         }

         // Switch the side to move
         SwitchSideToMove(sNullBoard);

         // Perform the null search with a narrow window (null window search)
         iNullScore = -Search(sNullBoard,
            argsGeneralMoves,
            -argiBeta,
            -argiBeta + 1);

         // Free up the created board using consistent memory management
         delete sNullBoard;
