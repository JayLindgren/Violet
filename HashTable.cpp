...
static void AllocateHashWithCount( BitBoard entryCount )
{
   ...
   if ( gsHashTable.mbbHash )
      free( gsHashTable.mbbHash );
   if ( gsHashTable.mbbHashTable )
...
void SetHashTableSizeBits( int bits )
{
   if ( bits < 1 )
      bits = 1;
   if ( bits > 30 )
      bits = 30;                         // guard against overflow / excessive allocations
...

{
   ...
   // Moving King or Rook
   if ( iPiece == dWhiteKing )
      bbLostRights |= ( 1ULL << 0 ) | ( 1ULL << 2 );
   if ( iPiece == dBlackKing )
      bbLostRights |= ( 1ULL << 1 ) | ( 1ULL << 3 );
   ...
   if ( bbRightsToRemove )
   {
      if ( bbRightsToRemove & ( 1ULL << 0 ) )
         argsBoard->bbHash ^= gsHashTable.vbbCasteling[ 0 ];
      if ( bbRightsToRemove & ( 1ULL << 1 ) )
         argsBoard->bbHash ^= gsHashTable.vbbCasteling[ 1 ];
      if ( bbRightsToRemove & ( 1ULL << 2 ) )
         argsBoard->bbHash ^= gsHashTable.vbbCasteling[ 2 ];
      if ( bbRightsToRemove & ( 1ULL << 3 ) )
         argsBoard->bbHash ^= gsHashTable.vbbCasteling[ 3 ];
   }
                            ...
                            struct GeneralMove *argsGeneralMoves )
{
   ...
   // 3. Castling Rights
   if ( argsBoard->bbCastle & ( 1ULL << 0 ) )
      argsBoard->bbHash ^= gsHashTable.vbbCasteling[ 0 ];
   if ( argsBoard->bbCastle & ( 1ULL << 1 ) )
      argsBoard->bbHash ^= gsHashTable.vbbCasteling[ 1 ];
   if ( argsBoard->bbCastle & ( 1ULL << 2 ) )
      argsBoard->bbHash ^= gsHashTable.vbbCasteling[ 2 ];
   if ( argsBoard->bbCastle & ( 1ULL << 3 ) )
      argsBoard->bbHash ^= gsHashTable.vbbCasteling[ 3 ];
