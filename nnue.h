...
#if defined (_WIN32) && defined(NNUE_DLL)
...
DLLExport void _CDECL nnue_init(
  ...
  bool silent = false               /** Suppress output if true */
);
...
/**
* Prefetch NNUE accumulator update.
* -------------------------------------------------
* Attempts to incrementally update the accumulator for the current position.
* Does nothing if full refresh is required (e.g. King move).
*/
DLLExport void _CDECL nnue_prefetch(
  NNUEdata ** nnue_data,              /** Pointer to NNUEdata* for current and previous plies */
  int* kingSquares                    /** Array of 2 integers: [0]=WhiteKingSq, [1]=BlackKingSq */
);
