...
DLLExport void _CDECL nnue_init(const char * evalFile, bool silent)
{
  ...
  if (!success) {
      if (!silent) printf("info string NNUE Error: Failed to load eval file: %s\n", evalFile);
      // Try absolute path as fallback relative to current dir
      char absPath[1024];
      if (_fullpath(absPath, evalFile, 1024) != NULL) {
           if (!silent) printf("info string NNUE Error: Tried absolute path: %s\n", absPath);
      }
  } else {
      if (!silent) printf("info string NNUE Success: Loaded eval file: %s\n", evalFile);
  }
}
