...
INLINE int neon_movemask( uint8x16_t v )
{
   ...
   return vgetq_lane_u8( (uint8x16_t)mask, 0 ) | ( vgetq_lane_u8( (uint8x16_t)mask, 8 ) << 8 );
}
#endif
#endif
...
INLINE bool update_accumulator( Position *pos )
{
   ...
   Accumulator *prevAcc;
   if ( ( !pos->nnue[ 1 ] || !( prevAcc = &pos->nnue[ 1 ]->accumulator )->computedAccumulation ) && ( !pos->nnue[ 2 ] || !( prevAcc = &pos->nnue[ 2 ]->accumulator )->computedAccumulation ) )
      return false;
...
{
   // nn-62ef826d1a6d.nnue (Stockfish 13 HalfKP network)
   if ( size != 21022697 )
      return false;

   // File size matches expected HalfKP network
   // Optionally could add magic number checks here
   // For now, trusting file size verification is sufficient
...
Interfaces
*/
DLLExport void _CDECL nnue_init( const char *evalFile, bool silent )
{
   bool success = load_eval_file( evalFile );
   if ( !success )
   {
      if ( !silent )
         printf( "info string NNUE Error: Failed to load eval file: %s\n", evalFile );
      // Try absolute path as fallback relative to current dir
      char absPath[ 1024 ];
      if ( _fullpath( absPath, evalFile, 1024 ) != NULL )
      {
         if ( !silent )
            printf( "info string NNUE Error: Tried absolute path: %s\n", absPath );
      }
   }
   else
   {
      if ( !silent )
         printf( "info string NNUE Success: Loaded eval file: %s\n", evalFile );
   }
}
...
DLLExport void _CDECL nnue_prefetch( NNUEdata **nnue, int *kingSquares )
{
   if ( !nnue[ 0 ] )
      return;

   // Check for King moves (which require full refresh and board state)
   // wking=1, bking=7
   int pc = nnue[ 0 ]->dirtyPiece.pc[ 0 ];
   if ( pc == 1 || pc == 7 )
      return;

   // Check previous ply if needed
   if ( !nnue[ 1 ] || !nnue[ 1 ]->accumulator.computedAccumulation )
   {
      if ( nnue[ 1 ] )
      {
         pc = nnue[ 1 ]->dirtyPiece.pc[ 0 ];
         if ( pc == 1 || pc == 7 )
            return;
      }
   }

   // Create dummy squares array for King lookups
   int squares[ 33 ];               // Size matches standard array
   squares[ 0 ] = kingSquares[ 0 ]; // White King
   squares[ 1 ] = kingSquares[ 1 ]; // Black King
   // Other squares are not needed for incremental updates (only King relative)
   ...
   pos.nnue[ 2 ] = nnue[ 2 ];
   pos.pieces    = nullptr;
   pos.squares   = squares; // Pass the dummy squares array
   pos.player    = 0;       // Irrelevant for accumulator update
