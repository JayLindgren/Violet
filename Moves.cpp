                     ...
                     struct GeneralMove *argsGeneralMoves )
{
   ...
   if ( argsBoard->siColorToMove == dWhite )
   {
      ...
      BitBoard bbOldEP           = argsBoard->bbEP;          // Save EP square
      ...
      argsBoard->bbEP            = bbOldEP; // Restore EP square
      // =========================== FIX END ===========================
                          ...
                          struct GeneralMove *argsGeneralMoves )
{
   ...
   {
      if ( argsBoard->siNumberOfMoves >= dNumberOfMoves )
         break; // Bounds check
                          ...
                          struct GeneralMove *argsGeneralMoves )
{
   ...
   // Loop over the rooks and generate moves using PEXT
   for ( int rookIndex = 0; rookIndex < numberOfRooks; rookIndex++ )
   {
      int      fromSquare  = vWhiteRooks[ rookIndex ];
      BitBoard rookAttacks = GetRookAttacksPEXT( fromSquare, argsBoard->bbAllPieces );

      // Update attack/move bitboards
      argsBoard->bbWhiteAttack |= rookAttacks;
      BitBoard rookMoves = rookAttacks & ~argsBoard->bbWhitePieces;
      argsBoard->bbWhiteMove |= rookMoves;

      // Generate actual legal moves
      while ( rookMoves )
      {
         unsigned long index;
         _BitScanForward64( &index, rookMoves );
         int toSquare = (int)index;
         rookMoves &= rookMoves - 1; // Clear LSB

         int targetPiece = argsBoard->mBoard[ toSquare ];

         argsBoard->siNumberOfMoves++;
         InitializeMoves( argsMoves, argsBoard->siNumberOfMoves - 1 );
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].bbFromSquare = SetBitToOne( 0, fromSquare );
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].bbToSquare   = SetBitToOne( 0, toSquare );
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].iFromSquare  = fromSquare;
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].iToSquare    = toSquare;
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].iPiece       = dWhiteRook;

         if ( targetPiece == dEmpty )
         {
         ...
         {
            argsMoves[ argsBoard->siNumberOfMoves - 1 ].iMoveType = dCapture;
            argsMoves[ argsBoard->siNumberOfMoves - 1 ].iCapture  = targetPiece;
            argsMoves[ argsBoard->siNumberOfMoves - 1 ].iScore    = argsGeneralMoves->msRookTakesPawn + ( targetPiece - dBlackPawn );
         }
      }
   }
}
                            ...
                            struct GeneralMove *argsGeneralMoves )
{
   ...
   // Find all white bishops
   int vWhiteBishops[ 10 ];
   int numberOfBishops = Find( argsBoard->bbWhiteBishop, vWhiteBishops, argsGeneralMoves );

   // Loop over the bishops and generate moves using PEXT
   for ( int bishopIndex = 0; bishopIndex < numberOfBishops; bishopIndex++ )
   {
      int      fromSquare    = vWhiteBishops[ bishopIndex ];
      BitBoard bishopAttacks = GetBishopAttacksPEXT( fromSquare, argsBoard->bbAllPieces );

      argsBoard->bbWhiteAttack |= bishopAttacks;
      BitBoard bishopMoves = bishopAttacks & ~argsBoard->bbWhitePieces;
      argsBoard->bbWhiteMove |= bishopMoves;

      while ( bishopMoves )
      {
         unsigned long index;
         _BitScanForward64( &index, bishopMoves );
         int toSquare = (int)index;
         ...
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].iFromSquare  = fromSquare;
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].iToSquare    = toSquare;
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].iPiece       = dWhiteBishop;
         ...
         else
         {
            argsMoves[ argsBoard->siNumberOfMoves - 1 ].iMoveType = dCapture;
            argsMoves[ argsBoard->siNumberOfMoves - 1 ].iCapture  = targetPiece;
            argsMoves[ argsBoard->siNumberOfMoves - 1 ].iScore    = argsGeneralMoves->msBishopTakesPawn + ( targetPiece - dBlackPawn );
         }
      }
   }
}
                           ...
                           struct GeneralMove *argsGeneralMoves )
{
   ...
   // Find all white queens
   int vWhiteQueens[ 10 ];
   int numberOfQueens = Find( argsBoard->bbWhiteQueen, vWhiteQueens, argsGeneralMoves );

   // Loop over the queens and generate moves using PEXT
   for ( int queenIndex = 0; queenIndex < numberOfQueens; queenIndex++ )
   {
      int      fromSquare   = vWhiteQueens[ queenIndex ];
      BitBoard queenAttacks = GetQueenAttacksPEXT( fromSquare, argsBoard->bbAllPieces );

      argsBoard->bbWhiteAttack |= queenAttacks;
      BitBoard queenMoves = queenAttacks & ~argsBoard->bbWhitePieces;
      argsBoard->bbWhiteMove |= queenMoves;

      while ( queenMoves )
      {
         unsigned long index;
         _BitScanForward64( &index, queenMoves );
         int toSquare = (int)index;
         ...
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].iFromSquare  = fromSquare;
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].iToSquare    = toSquare;
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].iPiece       = dWhiteQueen;
         ...
         else
         {
            argsMoves[ argsBoard->siNumberOfMoves - 1 ].iMoveType = dCapture;
            argsMoves[ argsBoard->siNumberOfMoves - 1 ].iCapture  = targetPiece;
            argsMoves[ argsBoard->siNumberOfMoves - 1 ].iScore    = argsGeneralMoves->msQueenTakesPawn + ( targetPiece - dBlackPawn );
         }
      }
   }
}
                          ...
                          struct GeneralMove *argsGeneralMoves )
{
   ...
   // Find all black rooks
   int vBlackRooks[ 10 ];
   int numberOfRooks = Find( argsBoard->bbBlackRook, vBlackRooks, argsGeneralMoves );

   // Loop over the rooks and generate moves using PEXT
   for ( int rookIndex = 0; rookIndex < numberOfRooks; rookIndex++ )
   {
      int      fromSquare  = vBlackRooks[ rookIndex ];
      BitBoard rookAttacks = GetRookAttacksPEXT( fromSquare, argsBoard->bbAllPieces );

      argsBoard->bbBlackAttack |= rookAttacks;
      BitBoard rookMoves = rookAttacks & ~argsBoard->bbBlackPieces;
      argsBoard->bbBlackMove |= rookMoves;
      ...
      while ( rookMoves )
      {
         ...
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].iFromSquare  = fromSquare;
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].iToSquare    = toSquare;
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].iPiece       = dBlackRook;
         ...
         else
         {
            argsMoves[ argsBoard->siNumberOfMoves - 1 ].iMoveType = dCapture;
            argsMoves[ argsBoard->siNumberOfMoves - 1 ].iCapture  = targetPiece;
            argsMoves[ argsBoard->siNumberOfMoves - 1 ].iScore    = argsGeneralMoves->msRookTakesPawn + ( targetPiece - dWhitePawn );
         }
      }
   }
}
                            ...
                            struct GeneralMove *argsGeneralMoves )
{
   ...
   // Find all black bishops
   int vBlackBishops[ 10 ];
   int numberOfBishops = Find( argsBoard->bbBlackBishop, vBlackBishops, argsGeneralMoves );

   // Loop over the bishops and generate moves using PEXT
   for ( int bishopIndex = 0; bishopIndex < numberOfBishops; bishopIndex++ )
   {
      int      fromSquare    = vBlackBishops[ bishopIndex ];
      BitBoard bishopAttacks = GetBishopAttacksPEXT( fromSquare, argsBoard->bbAllPieces );

      argsBoard->bbBlackAttack |= bishopAttacks;
      BitBoard bishopMoves = bishopAttacks & ~argsBoard->bbBlackPieces;
      argsBoard->bbBlackMove |= bishopMoves;
      ...
      while ( bishopMoves )
      {
         ...
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].iFromSquare  = fromSquare;
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].iToSquare    = toSquare;
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].iPiece       = dBlackBishop;
         ...
         else
         {
            argsMoves[ argsBoard->siNumberOfMoves - 1 ].iMoveType = dCapture;
            argsMoves[ argsBoard->siNumberOfMoves - 1 ].iCapture  = targetPiece;
            argsMoves[ argsBoard->siNumberOfMoves - 1 ].iScore    = argsGeneralMoves->msBishopTakesPawn + ( targetPiece - dWhitePawn );
         }
      }
   }
}
                           ...
                           struct GeneralMove *argsGeneralMoves )
{
   ...
   // Find all black queens
   int vBlackQueens[ 10 ];
   int numberOfQueens = Find( argsBoard->bbBlackQueen, vBlackQueens, argsGeneralMoves );

   // Loop over the queens and generate moves using PEXT
   for ( int queenIndex = 0; queenIndex < numberOfQueens; queenIndex++ )
   {
      int      fromSquare   = vBlackQueens[ queenIndex ];
      BitBoard queenAttacks = GetQueenAttacksPEXT( fromSquare, argsBoard->bbAllPieces );

      argsBoard->bbBlackAttack |= queenAttacks;
      BitBoard queenMoves = queenAttacks & ~argsBoard->bbBlackPieces;
      argsBoard->bbBlackMove |= queenMoves;
      ...
      while ( queenMoves )
      {
         ...
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].iFromSquare  = fromSquare;
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].iToSquare    = toSquare;
         argsMoves[ argsBoard->siNumberOfMoves - 1 ].iPiece       = dBlackQueen;
         ...
         else
         {
            argsMoves[ argsBoard->siNumberOfMoves - 1 ].iMoveType = dCapture;
            argsMoves[ argsBoard->siNumberOfMoves - 1 ].iCapture  = targetPiece;
            argsMoves[ argsBoard->siNumberOfMoves - 1 ].iScore    = argsGeneralMoves->msQueenTakesPawn + ( targetPiece - dWhitePawn );
         }
      }
   }
}
                     ...
                     struct GeneralMove *argsGeneralMoves )
{
   ...
   // Check for check status first to update board state before move generation
                    ...
                    struct GeneralMove *argsGeneralMoves )
{
   ...
   while ( iInputFlag )
   {
      ...
      if ( !strcmp( strMove, "quit" ) || !strcmp( strMove, "q" ) )
      {

         Destroy();
         exit( 0 );
      }
      else if ( !strcmp( strMove, "testpv" ) )
      {
         TestPVBug( argsBoard, argsGeneralMoves );
         continue;
      }
      else if ( !strcmp( strMove, "uci" ) )
      {
         SetInterfaceMode( dUCI );
         return;
      }
      else if ( !strcmp( strMove, "testmoveordering" ) )
      {
         TestMoveOrdering( argsBoard, argsGeneralMoves, false );
         continue;
      }
      else if ( !strcmp( strMove, "tunemoveordering" ) )
      {
         TuneMoveOrderingB( argsBoard, argsGeneralMoves );
         continue;
      }
      else if ( !strcmp( strMove, "testsee" ) )
      {
         TestSEE( argsBoard, argsGeneralMoves );
         continue;
      }
      else if ( !strcmp( strMove, "perf" ) )
      {
         PrintPerformanceStats();
         continue;
      }
      else if ( !strcmp( strMove, "testhash" ) )
      {
      ...
      {
         CommandFromInterface( strMove, argsBoard, argsGeneralMoves );
         continue;
      }
      else if ( !strcmp( strMove, "printboard" ) )
      {
         PrintBoard( argsBoard->mBoard );
         cout << "50 Move Counter: " << argsBoard->iFiftyMoveCounter << endl;
         continue;
      }
      else if ( !strcmp( strMove, "position" ) )
      {
         char line[ 1024 ];
         cin.getline( line, 1024 );
         string command = "position";
         command += line;
         PositionCommand( command, argsBoard, argsGeneralMoves );
         continue;
      }
      else if ( !strcmp( strMove, "undo" ) )
      {
      ...
      else if ( !strcmp( strMove, "printfen" ) )
      {
         PrintFEN( argsBoard, argsGeneralMoves );
         continue;
      }
      else if ( !strcmp( strMove, "checkdraw" ) )
      {
         if ( LookForDraw( argsBoard, argsGeneralMoves ) )
         {
            cout << "Draw Detected" << endl;
         }
         else
         {
            cout << "No Draw Detected" << endl;
         }
         continue;
      }
               ...
               char               *argstrMove )
{
   ...
   *sBoard = *argsBoard;
   ...
   // cjl
   // set the time for the search here.
   int iOldTime = GetSearchTimeInMiliSeconds();
   SetSearchTimeInMiliSeconds( dOneSecond );
                   ...
                   struct GeneralMove *argsGeneralMoves )
{
   ...
   if ( GetIsInBook() == dYes )
   {
      ...
      if ( iBookMove > -1 )
      {
         ...
         // UCI Output for Book Move
         if ( GetInterfaceMode() == dUCI || GetInterfaceMode() == dVFE )
         {
            // strMove already contains the algebraic move from CreateAlgebraicMove above
            // But CreateAlgebraicMove might pad it or format it?
            // It seems CreateAlgebraicMove produces "e2e4" or similar.
            // Let's ensure we trim it or pass it correctly.
            // strMove is char[64].
            std::string bestMoveStr = strMove;
            // Trim whitespace
            bestMoveStr.erase( bestMoveStr.find_last_not_of( " \n\r\t" ) + 1 );
            bestMoveStr.erase( 0, bestMoveStr.find_first_not_of( " \n\r\t" ) );
            SendBestMoveCommand( bestMoveStr );
         }
      }
   ...
   // UCI Output for Search Move
   if ( iBookMove == -1 && ( GetInterfaceMode() == dUCI || GetInterfaceMode() == dVFE ) )
   {
      // FIX: Validate PV before using it
      if ( argsBoard->vmPrincipalVariation[ 0 ][ 0 ].iFromSquare >= 0 &&
           argsBoard->vmPrincipalVariation[ 0 ][ 0 ].iFromSquare < 64 &&
           argsBoard->vmPrincipalVariation[ 0 ][ 0 ].iToSquare >= 0 &&
           argsBoard->vmPrincipalVariation[ 0 ][ 0 ].iToSquare < 64 )
      {
         char strBestMove[ 10 ];
         // Get the best move from the PV
         // argsBoard->vmPrincipalVariation[0][0] is the best move
         CreateAlgebraicMove( strBestMove, &argsBoard->vmPrincipalVariation[ 0 ][ 0 ], 0 );
         SendBestMoveCommand( strBestMove );
      }
      else
      {
         std::cerr << "ERROR: Invalid PV - no valid move to output!" << std::endl;
         // Output a null move as fallback
         SendBestMoveCommand( "0000" );
      }
   }
   ...
   if ( GetInterfaceMode() == dConsole )
   {
      ...
      if ( iBookMove == -1 )
      {
         ...
         int  iNumberOfCharacters = 0; // FIX: Move declaration outside if block
         char strMove[ 10 ];
         ...
         if ( argsBoard->vmPrincipalVariation[ 0 ][ 0 ].iFromSquare >= 0 &&
              ...
              argsBoard->vmPrincipalVariation[ 0 ][ 0 ].iToSquare >= 0 &&
              argsBoard->vmPrincipalVariation[ 0 ][ 0 ].iToSquare < 64 &&
              argsBoard->vmPrincipalVariation[ 0 ][ 0 ].iFromSquare != argsBoard->vmPrincipalVariation[ 0 ][ 0 ].iToSquare )
         {
            iNumberOfCharacters = PrintMove( argsBoard,
                                             argsGeneralMoves,
            ...
            // Create a marker for seeing if this is the perfect move.
            // cout << endl << "Violet move = " << strMove << endl << endl;

            // FIX: Make the move on the board (add moveNumber parameter = 0)
            MakeMove( &argsBoard->vmPrincipalVariation[ 0 ][ 0 ],
                      argsBoard,
                      argsGeneralMoves,
                      0 ); // moveNumber = 0 for PV move
         }
         else
         {
            cout << endl
                 << "ERROR: Search completed but PV is invalid - cannot make move!" << endl;
            cout << "PV[0][0]: from=" << argsBoard->vmPrincipalVariation[ 0 ][ 0 ].iFromSquare
                 << " to=" << argsBoard->vmPrincipalVariation[ 0 ][ 0 ].iToSquare << endl
                 << endl;
         }
         // cout << strMove << endl;

         // Print only the first iNumberOfCharacters characters of strMove
         cout << "Violet move = ";
         for ( int i = 0; i < iNumberOfCharacters; ++i )
         {
            std::cout << strMove[ i ];
         }
         std::cout << std::endl; // Add a newline for better formatting

         // I would like to print blank line and then print the move to be made here.
      }
   ...
   // NOTE: For Console mode, NON-BOOK moves are already made above (line 5122)
   // For book moves, we need to make them here for ALL modes
   if ( iBookMove != -1 )
   {

      // Make a book move (all modes - console and UCI)
      MakeMove( &vsMoveList[ iBookMove ],
                argsBoard,
                argsGeneralMoves,
                0 );
   }
   else if ( GetInterfaceMode() != dConsole )
   {

      // Make an out of book move (UCI mode only - console already made it at line 5122)
      MakeMove( &argsBoard->vmPrincipalVariation[ 0 ][ 0 ],
                argsBoard,
                argsGeneralMoves,
                0 );
   ...
   */
   ...
   {
      // Note: bestmove is already sent via SendBestMoveCommand earlier
      // No additional output needed here for UCI/VFE modes
   }
}
                 ...
                 struct GeneralMove *argsGeneralMoves )
{
   ...
   // Look for check (Verify if side that just moved left their King in check)
   if ( argsBoard->siColorToMove == dBlack )
   {
      // White just moved. Check if White King is attacked.
      if ( argsBoard->bbWhiteKing )
      {
         unsigned long kingIndex;
         _BitScanForward64( &kingIndex, argsBoard->bbWhiteKing );
         if ( IsSquareAttacked( argsBoard, argsGeneralMoves, (int)kingIndex, dBlack ) )
            argsBoard->siLegalMove = 0;
      }
      else
         argsBoard->siLegalMove = 0; // King captured/missing
   }

   if ( argsBoard->siColorToMove == dWhite )
   {
      // Black just moved. Check if Black King is attacked.
      if ( argsBoard->bbBlackKing )
      {
         unsigned long kingIndex;
         _BitScanForward64( &kingIndex, argsBoard->bbBlackKing );
         if ( IsSquareAttacked( argsBoard, argsGeneralMoves, (int)kingIndex, dWhite ) )
            argsBoard->siLegalMove = 0;
      }
      else
         argsBoard->siLegalMove = 0; // King captured/missing
   ...
   if ( ( argsBoard->sHistoryStack[ argsBoard->iMoveHistory ].iMoveType == dWhiteKingSideCastle ) &&
        ( argsBoard->siColorToMove == dBlack ) )
   {
      BitBoard mask = argsGeneralMoves->bbKingSideWhiteCastleArea;
      while ( mask )
      {
         unsigned long sq;
         _BitScanForward64( &sq, mask );
         mask &= mask - 1;
         if ( IsSquareAttacked( argsBoard, argsGeneralMoves, (int)sq, dBlack ) )
         {
            argsBoard->siLegalMove = 0;
            break;
         }
   ...
   if ( ( argsBoard->sHistoryStack[ argsBoard->iMoveHistory ].iMoveType == dWhiteQueenSideCastle ) &&
        ( argsBoard->siColorToMove == dBlack ) )
   {
      BitBoard mask = argsGeneralMoves->bbQueenSideWhiteCastleArea;
      while ( mask )
      {
         unsigned long sq;
         _BitScanForward64( &sq, mask );
   ...
   if ( ( argsBoard->sHistoryStack[ argsBoard->iMoveHistory ].iMoveType == dBlackKingSideCastle ) &&
        ( argsBoard->siColorToMove == dWhite ) )
   {
      BitBoard mask = argsGeneralMoves->bbKingSideBlackCastleArea;
      while ( mask )
      {
         unsigned long sq;
         _BitScanForward64( &sq, mask );
         mask &= mask - 1;
         if ( IsSquareAttacked( argsBoard, argsGeneralMoves, (int)sq, dWhite ) )
         {
            argsBoard->siLegalMove = 0;
            break;
         }
   ...
   if ( ( argsBoard->sHistoryStack[ argsBoard->iMoveHistory ].iMoveType == dBlackQueenSideCastle ) &&
        ( argsBoard->siColorToMove == dWhite ) )
   {
      BitBoard mask = argsGeneralMoves->bbQueenSideBlackCastleArea;
      while ( mask )
      {
         unsigned long sq;
         _BitScanForward64( &sq, mask );
                                 ...
                                 struct GeneralMove *argsGeneralMoves )
{
   ...
   // Find all white rooks
   int vWhiteRooks[ 10 ];
   int numberOfRooks = Find( argsBoard->bbWhiteRook, vWhiteRooks, argsGeneralMoves );

   // Loop over the rooks and generate ONLY attack moves using PEXT
   for ( int rookIndex = 0; rookIndex < numberOfRooks; rookIndex++ )
   {
      int      fromSquare  = vWhiteRooks[ rookIndex ];
      BitBoard rookAttacks = GetRookAttacksPEXT( fromSquare, argsBoard->bbAllPieces );
      argsBoard->bbWhiteAttack |= rookAttacks;                        // Update cumulative attacks
      BitBoard rookCaptures = rookAttacks & argsBoard->bbBlackPieces; // Only captures

      while ( rookCaptures )
      {
         unsigned long index;
         _BitScanForward64( &index, rookCaptures );
         int toSquare = (int)index;
                                   ...
                                   struct GeneralMove *argsGeneralMoves )
{
   ...
   // Loop over the bishops and generate ONLY attack moves using PEXT
   for ( int bishopIndex = 0; bishopIndex < numberOfBishops; bishopIndex++ )
   {
      int      fromSquare    = vWhiteBishops[ bishopIndex ];
      BitBoard bishopAttacks = GetBishopAttacksPEXT( fromSquare, argsBoard->bbAllPieces );
      argsBoard->bbWhiteAttack |= bishopAttacks; // Update cumulative attacks
      BitBoard bishopCaptures = bishopAttacks & argsBoard->bbBlackPieces;

      while ( bishopCaptures )
      {
         unsigned long index;
         _BitScanForward64( &index, bishopCaptures );
         int toSquare = (int)index;
                                  ...
                                  struct GeneralMove *argsGeneralMoves )
{
   ...
   //  Loop over the queens and generate ONLY attack moves using PEXT
   for ( int queenIndex = 0; queenIndex < numberOfQueens; queenIndex++ )
   {
      int      fromSquare   = vWhiteQueens[ queenIndex ];
      BitBoard queenAttacks = GetQueenAttacksPEXT( fromSquare, argsBoard->bbAllPieces );
      argsBoard->bbWhiteAttack |= queenAttacks; // Update cumulative attacks
      BitBoard queenCaptures = queenAttacks & argsBoard->bbBlackPieces;

      while ( queenCaptures )
      {
         unsigned long index;
         _BitScanForward64( &index, queenCaptures );
         int toSquare = (int)index;
                                 ...
                                 struct GeneralMove *argsGeneralMoves )
{
   ...
   for ( int rookIndex = 0; rookIndex < numberOfRooks; rookIndex++ )
   {
      int      fromSquare  = vBlackRooks[ rookIndex ];
      BitBoard rookAttacks = GetRookAttacksPEXT( fromSquare, argsBoard->bbAllPieces );
      argsBoard->bbBlackAttack |= rookAttacks; // Update cumulative attacks
      BitBoard rookCaptures = rookAttacks & argsBoard->bbWhitePieces;
                                   ...
                                   struct GeneralMove *argsGeneralMoves )
{
   ...
   for ( int bishopIndex = 0; bishopIndex < numberOfBishops; bishopIndex++ )
   {
      int      fromSquare    = vBlackBishops[ bishopIndex ];
      BitBoard bishopAttacks = GetBishopAttacksPEXT( fromSquare, argsBoard->bbAllPieces );
      argsBoard->bbBlackAttack |= bishopAttacks; // Update cumulative attacks
      BitBoard bishopCaptures = bishopAttacks & argsBoard->bbWhitePieces;
                                  ...
                                  struct GeneralMove *argsGeneralMoves )
{
   ...
   for ( int queenIndex = 0; queenIndex < numberOfQueens; queenIndex++ )
   {
      int      fromSquare   = vBlackQueens[ queenIndex ];
      BitBoard queenAttacks = GetQueenAttacksPEXT( fromSquare, argsBoard->bbAllPieces );
      argsBoard->bbBlackAttack |= queenAttacks; // Update cumulative attacks
      BitBoard queenCaptures = queenAttacks & argsBoard->bbWhitePieces;
                      ...
                      int  *iRow )
{
   ...
   *iRow = (int)( tolower( *strRow ) - 97 );
   ...
   if ( ( *iRow < 0 ) || ( *iRow > 8 ) )
   {
      ...
      cout << "ConvertRowToInt failed. Char: " << *strRow << " (" << (int)*strRow << ")" << endl;
      // system( "pause" );
   }
}
                      ...
                      int  *iCol )
{
   ...
   *iCol = ( (int)*strCol - 49 );
   ...
   if ( ( *iCol < 0 ) || ( *iCol > 8 ) )
   {
      ...
      cout << "ConvertColToInt failed. Char: " << *strCol << " (" << (int)*strCol << ")" << endl;
      // system( "pause" );
   }
}
                   ...
                   char               *argstrMove )
{
   ...
   if ( iFoundMove == 0 )
   {
      ...
      cout << "Move was not found: " << argstrMove << endl;
      cout << "Move was not found, try again and look for degeneracy." << endl;
               ...
               int                 iMoveNumber )
{
   ...
   // Prefetch NNUE accumulator for the current position to ensure incremental updates.
   // Only do this for multithreaded search to avoid overhead in single-threaded mode.
   if ( GetNumberOfThreads() > 1 )
   {
      PrefetchNNUE( argsBoard );
   }
   ...
   argsBoard->sHistoryStack[ argsBoard->iMoveHistory ].bbEPSquare   = argsBoard->bbEP;
   ...
   // NNUE Update Logic
   {
      NNUEdata *nnue                         = &argsBoard->sNNUEHistory[ argsBoard->iMoveHistory ];
      nnue->accumulator.computedAccumulation = 0; // Invalidate accumulator
      DirtyPiece *dp                         = &nnue->dirtyPiece;
      dp->dirtyNum                           = 0;

      int from    = argsMoves[ iMoveNumber ].iFromSquare;
      int to      = argsMoves[ iMoveNumber ].iToSquare;
      int piece   = argsMoves[ iMoveNumber ].iPiece;
      int capture = argsBoard->mBoard[ to ]; // Capture is on 'to' square
      int type    = argsMoves[ iMoveNumber ].iMoveType;

      // Mapping
      const int violetToNNUE[ 13 ] = { 0, 6, 3, 5, 4, 2, 1, 12, 9, 11, 10, 8, 7 };

      // Handle Capture
      if ( capture != dEmpty && type != dEnPassant )
      {
         dp->pc[ dp->dirtyNum ]   = violetToNNUE[ capture ];
         dp->from[ dp->dirtyNum ] = to;
         dp->to[ dp->dirtyNum ]   = 64; // Remove
         dp->dirtyNum++;
      }

      // Handle Move / Promotion
      if ( type == dPromote || type == dCaptureAndPromote )
      {
         // Remove Pawn
         int pawnPiece            = ( argsBoard->siColorToMove == dWhite ) ? dWhitePawn : dBlackPawn;
         dp->pc[ dp->dirtyNum ]   = violetToNNUE[ pawnPiece ];
         dp->from[ dp->dirtyNum ] = from;
         dp->to[ dp->dirtyNum ]   = 64;
         dp->dirtyNum++;

         // Add Promoted Piece
         dp->pc[ dp->dirtyNum ]   = violetToNNUE[ piece ];
      ...
      // Handle Castling (Rook part)
      if ( type == dWhiteKingSideCastle )
      {
         dp->pc[ dp->dirtyNum ]   = violetToNNUE[ dWhiteRook ];
         dp->from[ dp->dirtyNum ] = dH1;
         dp->to[ dp->dirtyNum ]   = dF1;
         dp->dirtyNum++;
      }
      else if ( type == dWhiteQueenSideCastle )
      {
         dp->pc[ dp->dirtyNum ]   = violetToNNUE[ dWhiteRook ];
         dp->from[ dp->dirtyNum ] = dA1;
         dp->to[ dp->dirtyNum ]   = dD1;
         dp->dirtyNum++;
      }
      else if ( type == dBlackKingSideCastle )
      {
         dp->pc[ dp->dirtyNum ]   = violetToNNUE[ dBlackRook ];
         dp->from[ dp->dirtyNum ] = dH8;
         dp->to[ dp->dirtyNum ]   = dF8;
         dp->dirtyNum++;
      }
      else if ( type == dBlackQueenSideCastle )
      {
         dp->pc[ dp->dirtyNum ]   = violetToNNUE[ dBlackRook ];
         dp->from[ dp->dirtyNum ] = dA8;
         dp->to[ dp->dirtyNum ]   = dD8;
         dp->dirtyNum++;
      }
      ...
      if ( type == dEnPassant )
      {
         int captureSquare = ( argsBoard->siColorToMove == dWhite ) ? ( to - 8 ) : ( to + 8 );
         int capturedPawn  = ( argsBoard->siColorToMove == dWhite ) ? dBlackPawn : dWhitePawn;

         dp->pc[ dp->dirtyNum ]   = violetToNNUE[ capturedPawn ];
         dp->from[ dp->dirtyNum ] = captureSquare;
         dp->to[ dp->dirtyNum ]   = 64;
         dp->dirtyNum++;
      }
   }
   ...
   // Update the EP square on the board for the next move.
   ...
   // Store the OLD fifty-move counter in history BEFORE updating it
   argsBoard->sHistoryStack[ argsBoard->iMoveHistory ].iFiftyMoveCounter = argsBoard->iFiftyMoveCounter;

   // Increment the halfmove counter
   argsBoard->iFiftyMoveCounter++;

   // Reset counter on pawn move
   if ( argsMoves[ iMoveNumber ].iPiece == dWhitePawn ||
        argsMoves[ iMoveNumber ].iPiece == dBlackPawn )
   {
      argsBoard->iFiftyMoveCounter = 0;
   }

   // Reset counter on any capture (including en passant)
   if ( capturedPiece != dEmpty ||
        argsMoves[ iMoveNumber ].iMoveType == dEnPassant )
   {
      argsBoard->iFiftyMoveCounter = 0;
   }
   // ========== END 50-MOVE RULE COUNTER ==========
               ...
               struct GeneralMove *argsGeneralMoves )
{
   ...
   */
   ...
   {
      argsBoard->bbHash = argsBoard->sHistoryStack[ iMoveNumber - 1 ].bbHash;
   }
   ...
   argsBoard->iFiftyMoveCounter = argsBoard->sHistoryStack[ iMoveNumber ].iFiftyMoveCounter;
   ...
   else
   {
      ...
      case dWhitePawn:
      {
         ...
         if ( argsBoard->sHistoryStack[ iMoveNumber ].iMoveType == dEnPassant )
         {
            ...
            int iPosition = 0;
            int vPosition[ 1 ];
            iPosition                               = Find( argsBoard->sHistoryStack[ iMoveNumber ].bbEPSquare,
                                                            vPosition,
                                                            argsGeneralMoves );
            ...
            if ( !CheckBoard( argsBoard ) )
            {
               ...
               // PrintBitBoard( argsBoard->sHistoryStack[ iMoveNumber ].bbFromSquare );
               // PrintBitBoard( argsBoard->sHistoryStack[ iMoveNumber ].bbToSquare );
               // PrintBitBoard( argsBoard->sHistoryStack[ iMoveNumber ].bbEPSquare );
               // cout << "Hash = " << GetHash();
            }
         }
...
void InitializeMoveDebug()
{
   ...
   gofDebugMoves << endl;
   gofDebugMoves << "Move log started." << endl
                 << endl;
}
                                ...
                                struct GeneralMove *argsGeneralMoves )
{
   ...
   // Loop over the rooks and generate ONLY quiet moves using PEXT
   for ( int rookIndex = 0; rookIndex < numberOfRooks; rookIndex++ )
   {
      int      fromSquare  = vWhiteRooks[ rookIndex ];
      BitBoard rookAttacks = GetRookAttacksPEXT( fromSquare, argsBoard->bbAllPieces );
      BitBoard rookQuiets  = rookAttacks & ~argsBoard->bbAllPieces; // Empty squares only

      while ( rookQuiets )
      {
         unsigned long index;
         _BitScanForward64( &index, rookQuiets );
         int toSquare = (int)index;
         rookQuiets &= rookQuiets - 1;
                                  ...
                                  struct GeneralMove *argsGeneralMoves )
{
   ...
   // Loop over the bishops and generate ONLY quiet moves using PEXT
   for ( int bishopIndex = 0; bishopIndex < numberOfBishops; bishopIndex++ )
   {
      int      fromSquare    = vWhiteBishops[ bishopIndex ];
      BitBoard bishopAttacks = GetBishopAttacksPEXT( fromSquare, argsBoard->bbAllPieces );
      BitBoard bishopQuiets  = bishopAttacks & ~argsBoard->bbAllPieces;

      while ( bishopQuiets )
      {
         unsigned long index;
         _BitScanForward64( &index, bishopQuiets );
         int toSquare = (int)index;
         bishopQuiets &= bishopQuiets - 1;
                                 ...
                                 struct GeneralMove *argsGeneralMoves )
{
   ...
   // Loop over the queens and generate ONLY quiet moves using PEXT
   for ( int queenIndex = 0; queenIndex < numberOfQueens; queenIndex++ )
   {
      int      fromSquare   = vWhiteQueens[ queenIndex ];
      BitBoard queenAttacks = GetQueenAttacksPEXT( fromSquare, argsBoard->bbAllPieces );
      BitBoard queenQuiets  = queenAttacks & ~argsBoard->bbAllPieces;

      while ( queenQuiets )
      {
         unsigned long index;
         _BitScanForward64( &index, queenQuiets );
         int toSquare = (int)index;
         queenQuiets &= queenQuiets - 1;
                                ...
                                struct GeneralMove *argsGeneralMoves )
{
   ...
   for ( int rookIndex = 0; rookIndex < numberOfRooks; rookIndex++ )
   {
      int      fromSquare  = vBlackRooks[ rookIndex ];
      BitBoard rookAttacks = GetRookAttacksPEXT( fromSquare, argsBoard->bbAllPieces );
      BitBoard rookQuiets  = rookAttacks & ~argsBoard->bbAllPieces;
...
// Returns 1 if attacked, 0 otherwise.
//
                      ...
                      int                 siSide )
{
   ...
   if ( siSide == dWhite )
   {
      ...
      {
         int iCand = iSquare - 9;
         if ( iCand >= 0 && ( argsBoard->bbWhitePawn & ( 1ULL << iCand ) ) )
            return 1;
      }
   ...
   else
   {
      ...
      {
         int iCand = iSquare + 9;
         if ( iCand < 64 && ( argsBoard->bbBlackPawn & ( 1ULL << iCand ) ) )
            return 1;
      }
   ...
   if ( bbKnights & argsGeneralMoves->bbNMove[ iSquare ] )
      return 1;

   // 3. Bishops/Queens (Diagonal) - PEXT
   ...
   if ( bbBishops )
   {
      BitBoard bishopAttacks = GetBishopAttacksPEXT( iSquare, argsBoard->bbAllPieces );
      if ( bishopAttacks & bbBishops )
         return 1;
   }

   // 4. Rooks/Queens (Orthogonal) - PEXT
   BitBoard bbRooks = ( siSide == dWhite ) ? ( argsBoard->bbWhiteRook | argsBoard->bbWhiteQueen ) : ( argsBoard->bbBlackRook | argsBoard->bbBlackQueen );
   if ( bbRooks )
   {
      BitBoard rookAttacks = GetRookAttacksPEXT( iSquare, argsBoard->bbAllPieces );
      if ( rookAttacks & bbRooks )
         return 1;
   }

   // 5. King
   BitBoard bbKing = ( siSide == dWhite ) ? argsBoard->bbWhiteKing : argsBoard->bbBlackKing;
   if ( bbKing & argsGeneralMoves->bbKMove[ iSquare ] )
      return 1;
