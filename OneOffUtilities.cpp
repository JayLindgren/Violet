...
{
   // Copy primitive fields and small arrays
   // Everything up to mBoard is safe to copy directly if it's contiguous,
   // but safe to just copy fields individually to avoid padding issues.
   // Struct copy of the whole thing is what we want to avoid.

   // 1. Bitboards & Game State (Start of struct)
   dest->bbWhitePawn   = src->bbWhitePawn;
   dest->bbWhiteRook   = src->bbWhiteRook;
   dest->bbWhiteKnight = src->bbWhiteKnight;
   dest->bbWhiteBishop = src->bbWhiteBishop;
   dest->bbWhiteQueen  = src->bbWhiteQueen;
   dest->bbWhiteKing   = src->bbWhiteKing;

   dest->bbBlackPawn   = src->bbBlackPawn;
   dest->bbBlackRook   = src->bbBlackRook;
   dest->bbBlackKnight = src->bbBlackKnight;
   dest->bbBlackBishop = src->bbBlackBishop;
   dest->bbBlackQueen  = src->bbBlackQueen;
   dest->bbBlackKing   = src->bbBlackKing;

   dest->bbAllPieces   = src->bbAllPieces;
   dest->bbWhitePieces = src->bbWhitePieces;
   dest->bbBlackPieces = src->bbBlackPieces;

   dest->bbWhiteAttack = src->bbWhiteAttack;
   dest->bbWhiteMove   = src->bbWhiteMove;
   dest->bbBlackAttack = src->bbBlackAttack;
   dest->bbBlackMove   = src->bbBlackMove;

   dest->bbWhiteKingMove   = src->bbWhiteKingMove;
   dest->bbWhiteKingAttack = src->bbWhiteKingAttack;
   dest->bbBlackKingMove   = src->bbBlackKingMove;
   dest->bbBlackKingAttack = src->bbBlackKingAttack;

   dest->siNumberOfMoves = src->siNumberOfMoves;

   dest->iHasWhiteCastled = src->iHasWhiteCastled;
   dest->iHasBlackCastled = src->iHasBlackCastled;
   dest->bbCastle         = src->bbCastle;

   dest->bbCheck = src->bbCheck;

   dest->siColorToMove = src->siColorToMove;
   dest->bbEP          = src->bbEP;
   dest->bbHash        = src->bbHash;

   // 2. The Board Array (64 ints)
   memcpy( dest->mBoard, src->mBoard, sizeof( int ) * dNumberOfSquares );

   // 3. History
   dest->iMoveHistory = src->iMoveHistory;

   // Only copy the valid history
   // Safety check for history bounds
   int historyLimit = src->iMoveHistory + 1;
   if ( historyLimit > dNumberOfMoveHistory )
      historyLimit = dNumberOfMoveHistory;

   memcpy( dest->sHistoryStack, src->sHistoryStack, sizeof( struct HistoryStack ) * historyLimit );

   // 4. NNUE History
   // This is the BIG saving. Only copy what we need.
   // The accumulator for the current position is at iMoveHistory.
   // We likely need a few previous ones for incremental update?
   // Actually, we usually just need the current accumulator to start search.
   // But let's copy up to current history to be safe and consistent with sHistoryStack.
   memcpy( dest->sNNUEHistory, src->sNNUEHistory, sizeof( NNUEdata ) * historyLimit );

   // 5. Search State
   dest->siLegalMove          = src->siLegalMove;
   dest->iNumberOfPlys        = src->iNumberOfPlys;
   dest->iMaxPlys             = src->iMaxPlys;
   dest->iMaxPlysReached      = src->iMaxPlysReached;
   dest->iFiftyMoveCounter    = src->iFiftyMoveCounter;
   dest->iBestMove            = src->iBestMove;
   dest->iMoveOrder           = src->iMoveOrder;
   dest->iLastMoveNull        = src->iLastMoveNull;
   dest->iUseNullVerification = src->iUseNullVerification;

   dest->cStart  = src->cStart;
   dest->cFinish = src->cFinish;

   dest->iPlayGame       = src->iPlayGame;
   dest->siComputerColor = src->siComputerColor;

   // 6. Principal Variation
   // Thread start should reset PV, so no need to copy the matrix.
   // Just reset lengths.
   memset( dest->viPrincipalVariationLength, 0, sizeof( int ) * dNumberOfPlys );
}
               ...
               struct GeneralMove *argsGeneralMoves )
{
   ...
   for ( int iTestIndex = 0; iTestIndex < numTests; iTestIndex++ )
   {
      ...
      cout << "----------------------------------------" << endl;
      cout << "FEN: " << testPositions[ iTestIndex ] << endl
           << endl;
      ...
      cout << endl
           << "TEST A: Hash Make/Unmake Consistency" << endl;
      ...
      cout << endl
           << "TEST B: Hash Table Storage/Retrieval" << endl;
      ...
      {
         cout << endl
              << "TEST C: Castling Rights Distinction" << endl;
      ...
      if ( epPos == string::npos ) // Means there IS an EP square (e.g. " d6 ")
      {
         ...
         cout << endl
              << "TEST D: En Passant Distinction" << endl;
      ...
      cout << endl
           << "TEST E: Side to Move Hash Impact" << endl;
                              ...
                              struct GeneralMove *argsGeneralMoves )
{
   ...
   MakeMoveStr( "Nf3" );
   if ( LookForDraw( argsBoard, argsGeneralMoves ) )
      cout << "FAIL: Draw after Nf3" << endl;

   // Move 1... Nc6
   MakeMoveStr( "Nc6" );
   if ( LookForDraw( argsBoard, argsGeneralMoves ) )
      cout << "FAIL: Draw after Nc6" << endl;

   // Move 2: Ng1
   MakeMoveStr( "Ng1" );
   if ( LookForDraw( argsBoard, argsGeneralMoves ) )
      cout << "FAIL: Draw after Ng1" << endl;
...
//
//
void TestSingularExtension( struct Board       *argsBoard,
                            struct GeneralMove *argsGeneralMoves )
{
   cout << "========================================" << endl;
   cout << "SINGULAR EXTENSION ISOLATION TEST" << endl;
   cout << "========================================" << endl;
   cout << endl;

   // The Arasan Break Position
   // Only g5 wins. All other moves draw.
   const char *fen = "r3k3/1p4p1/1Bb1Bp1p/P1p1bP1P/2Pp2P1/3P4/5K2/4R3 w - - 0 1";

   cout << "Position: Arasan Break (locked endgame)" << endl;
   cout << "FEN: " << fen << endl;
   cout << "Expected Best Move: g5 (Pawn Push)" << endl;
   cout << "Challenge: g5 is the ONLY winning attempt." << endl;
   cout << "All other moves (e.g., Kg3, Re2) lead to fortress/draw." << endl;
   cout << "----------------------------------------" << endl;

   // Load the position
   ReadFEN( fen, argsBoard, argsGeneralMoves, 2 ); // 2 = Parse string directly

   // Display the board
   PrintBoard( argsBoard->mBoard );
   cout << endl;

   // Use the search depth already set by GameControl (Style configuration)
   int iTargetDepth = GetSearchDepth();
   ...
   cout << "Searching at depth " << iTargetDepth << "..." << endl;
   cout << "Singular Extensions: Active (MinDepth=" << dSingularMinDepth
        << ", Margin=" << dSingularMargin << ")" << endl;
   cout << endl;

   // Start the search
   int iAlpha = -99999;
   int iBeta  = 99999;
   int iScore = StartSearch( argsBoard, argsGeneralMoves, iAlpha, iBeta );

   // Get the best move
   Move bestMove = argsBoard->vmPrincipalVariation[ 0 ][ 0 ];
   char strMove[ 10 ];
   int  iMoveLen       = PrintMove( argsBoard, argsGeneralMoves, &bestMove, strMove );
   strMove[ iMoveLen ] = '\0'; // Manual null termination

   // Report results
   cout << "----------------------------------------" << endl;
   cout << "RESULTS" << endl;
   cout << "----------------------------------------" << endl;
   cout << " Best Move: " << strMove << endl;
   cout << " Score:     " << iScore << endl;
   cout << " Nodes:     " << FormatWithCommas( GetTotalNodes() ) << endl;
   cout << endl;

   // Check if we found the singular break
   std::string moveStr( strMove );
   if ( moveStr.find( "g5" ) != std::string::npos )
   {
      cout << "[SUCCESS] Violet found the singular break!" << endl;
      cout << "Singular Extensions are working correctly." << endl;
   }
   else
   {
      cout << "[FAILURE] Violet played: " << strMove << endl;
      cout << "This is likely a waiting move (e.g., Kg3, Re2)." << endl;
      cout << endl;
      cout << "DIAGNOSIS:" << endl;
      cout << "- SE may not be triggering frequently enough" << endl;
      cout << "- Try lowering dSingularMargin (currently 3)" << endl;
      cout << "- Try lowering dSingularMinDepth (currently 6)" << endl;
      cout << "- Verify hash move is being stored correctly" << endl;
   }
   cout << "========================================" << endl;
   cout << endl;
