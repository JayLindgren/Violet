...
//      ifstream ifFEN( strFileName );
   ...
   else if( argiInitialBoardFlag == 2 )
   {
      ...
      if (strPointer1 && strPointer2 && strPointer3 && strPointer4)
      {
          ...
          if ((strPointer4 - strPointer3) < 64) str4[strPointer4 - strPointer3] = '\0';
      }
      else
      {
                          ...
                          char * fenString)
{
    ...
    for (int iCol = 7; iCol >= 0; iCol--)
    {
        ...
        for (int iRow = 0; iRow < 8; iRow++)
        {
            ...
            else
            {
                ...
                switch (piece)
                {
                    case dWhitePawn:   buffer[bufferPos++] = 'P'; break;
                    case dWhiteRook:   buffer[bufferPos++] = 'R'; break;
                    case dWhiteKnight: buffer[bufferPos++] = 'N'; break;
                    case dWhiteBishop: buffer[bufferPos++] = 'B'; break;
                    case dWhiteQueen:  buffer[bufferPos++] = 'Q'; break;
                    case dWhiteKing:   buffer[bufferPos++] = 'K'; break;
                    case dBlackPawn:   buffer[bufferPos++] = 'p'; break;
                    case dBlackRook:   buffer[bufferPos++] = 'r'; break;
                    case dBlackKnight: buffer[bufferPos++] = 'n'; break;
                    case dBlackBishop: buffer[bufferPos++] = 'b'; break;
                    case dBlackQueen:  buffer[bufferPos++] = 'q'; break;
                    case dBlackKing:   buffer[bufferPos++] = 'k'; break;
                }
            }
        }
    ...
    if (argsBoard->bbCastle & 0x01) { buffer[bufferPos++] = 'K'; hasCastling = 1; }
    if (argsBoard->bbCastle & 0x04) { buffer[bufferPos++] = 'Q'; hasCastling = 1; }
    if (argsBoard->bbCastle & 0x02) { buffer[bufferPos++] = 'k'; hasCastling = 1; }
    if (argsBoard->bbCastle & 0x08) { buffer[bufferPos++] = 'q'; hasCastling = 1; }
