...
// Forward declarations for initialization helpers
...
// NOTE: CalculateRooks, CalculateBishops, CalculateQueen commented out (arrays removed - using PEXT)
...
void GenerateGeneralMove( struct GeneralMove *sGeneralMove )
{
   ...
   int iRow = 0;
   int iCol = 1;
   // int iNNUEInitialized;           // Flag: 1 if NNUE is loaded, 0 otherwise
   // char strNNUEFilePath[256];      // Store the NNUE file path

   // Calcualte the Selectors.
   CalculateSelectors( sGeneralMove );

   // Initialize Opening Book Score Masks
   // White Score: Bits 0-19
   sGeneralMove->iWhiteScoreShift = 0;
   sGeneralMove->bbWhiteScore     = ( 1ULL << dNumberOfBitsPerScore ) - 1;

   // Black Score: Bits 20-39
   sGeneralMove->iBlackScoreShift = dNumberOfBitsPerScore;
   sGeneralMove->bbBlackScore     = ( ( 1ULL << dNumberOfBitsPerScore ) - 1 ) << dNumberOfBitsPerScore;

   // Draw Score: Bits 40-59
   sGeneralMove->iDrawScoreShift = 2 * dNumberOfBitsPerScore;
   sGeneralMove->bbDrawScore     = ( ( 1ULL << dNumberOfBitsPerScore ) - 1 ) << ( 2 * dNumberOfBitsPerScore );
   ...
   for ( rowIndex = 0; rowIndex < 8; rowIndex++ )
   {
      ...
      for ( colIndex = 0; colIndex < 8; colIndex++ )
      {
         ...
         // NOTE: Rook, Bishop, and Queen move arrays removed - replaced by PEXT
         // Initialization commented out (arrays no longer exist in GeneralMove struct)
         /*
         ...
         */
         ...
         // Set up the pieces.
         // NOTE: Rook, Bishop, Queen initialization functions commented out
         // These arrays are now calculated on-the-fly using PEXT (see MoveGen.h)
...
#if dUseNNUE
   // Store the NNUE file path from Definitions.h
   // strncpy(sGeneralMove->strNNUEFilePath, dNNUEFile, sizeof(sGeneralMove->strNNUEFilePath) - 1);
   strncpy_s( sGeneralMove->strNNUEFilePath, sizeof( sGeneralMove->strNNUEFilePath ), dNNUEFile, _TRUNCATE );
   sGeneralMove->strNNUEFilePath[ sizeof( sGeneralMove->strNNUEFilePath ) - 1 ] = '\0'; // Ensure null termination

   // Initialize NNUE library with the network file
   static std::once_flag nnue_init_flag;
   std::call_once( nnue_init_flag, [ sGeneralMove ]()
                   {
          bool silent = (GetInterfaceMode() == dUCI);
          nnue_init(sGeneralMove->strNNUEFilePath, silent); } );

   // Mark as successfully initialized
   sGeneralMove->iNNUEInitialized = dYes;

// Optional: Print confirmation in debug/console mode
#ifndef NDEBUG
   cout << "NNUE initialized successfully with file: " << sGeneralMove->strNNUEFilePath << endl;
#endif
#else
   // NNUE is disabled, mark as not initialized
   sGeneralMove->iNNUEInitialized     = dNo;
   sGeneralMove->strNNUEFilePath[ 0 ] = '\0';
#endif
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
//
void CalculateKing( struct GeneralMove *sGeneralMove,
                    int                 rowIndex,
                    int                 colIndex,
                    int                 square )
//
...
////////////////////////////////////////////////////////////////////////////
//
// NOTE: CalculateQueen function commented out (uses removed arrays - replaced by PEXT)
/*
void CalculateQueen( struct GeneralMove *sGeneralMove,
                     int                 rowIndex,
                     int                 colIndex,
                     int                 square )
{
...
// NOTE: CalculateBishops function commented out (uses removed arrays - replaced by PEXT)
/*
void CalculateBishops( struct GeneralMove *sGeneralMove,
                       int                 rowIndex,
                       int                 colIndex,
                       int                 square )
{
...
// NOTE: CalculateRooks function commented out (uses removed arrays - replaced by PEXT)
/*
void CalculateRooks( struct GeneralMove *sGeneralMove,
                     int                 rowIndex,
                     int                 colIndex,
                     int                 square )
{
...
////////////////////////////////////////////////////////////////////////////
//
void CalculateKnights( struct GeneralMove *sGeneralMove,
                       int                 rowIndex,
                       int                 colIndex,
                       int                 square )
//
